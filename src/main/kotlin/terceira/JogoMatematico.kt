...
produto entre os dois dígitos. Ela pediu para seu amigo Marcelo, que é bom em programação, para criar um
programa para que encontre a solução para cada uma das sequências que Paula lhe apresentar.
...
fun main(args: Array<String>) {
    ...
    for (i in 1..N) {
        ...
        val N1 = input[0].toString().toInt()
        val L1 = input[1]
        val N2 = input[2].toString().toInt()

        when {
            N1 == N2 -> println(N1 * N2)
            L1.isUpperCase() -> println(N2 - N1)
            else -> println(N1 + N2)
        }
    }
}